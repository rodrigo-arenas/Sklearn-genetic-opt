"""Generator for examples/advanced-rf.md.

A deep dive into the advanced optimizer controls — smart initialization, warm
starts, diversity control, fitness sharing, local search, and adaptive
schedules — applied to a RandomForestClassifier on a deliberately non-trivial
make_classification dataset. The headline is the reliable, honest win: the
GA-tuned forest beats the default-hyperparameter forest on a held-out test set.
Every number and image in the page comes from real execution.
"""
...
nb = Notebook(
    path="examples/advanced-rf.md",
    title="Advanced Random Forest Tuning",
    description=(
        "A guided tour of the advanced optimizer controls — smart "
        "initialization, warm starts, diversity control, fitness sharing, "
        "local search, and adaptive schedules — that tune a RandomForestClassifier "
        "to beat its own defaults, with real telemetry and convergence plots."
    ),
    intro=(
        "This example is a guided tour of the *advanced* optimizer controls in "
        "`sklearn-genetic-opt`. We tune a `RandomForestClassifier` on a "
        "deliberately hard classification problem, then read the search telemetry "
        "(`fit_stats_`, `history`) and convergence plots to see how the controls "
        "shape the run.\n\n"
        "The headline is the reliable win: a **GA-tuned forest beats the "
        "default-hyperparameter forest** on an untouched test set. Defaults are a "
        "single fixed point in a large parameter space; the genetic search "
        "explores that space under cross-validation and lands somewhere better."
    ),
)
