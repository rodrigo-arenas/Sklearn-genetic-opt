"""Generator for tutorials/isolation-forest.md.

Tune an ``IsolationForest`` with ``GASearchCV`` using a custom scorer built on
``score_samples`` (negated so higher = more anomalous, matching ``y=1`` for
outliers). A 2D synthetic dataset — two Gaussian blobs plus uniform outliers —
keeps the anomaly-score surface interpretable as contour plots. The honest win:
the GA-tuned forest beats the default on ROC AUC.
"""
...
nb = Notebook(
    path="tutorials/isolation-forest.md",
    title="Tuning Isolation Forest With GASearchCV",
    description=(
        "Tune IsolationForest's four key hyperparameters using a genetic algorithm "
        "on a labeled anomaly dataset with a custom score_samples scorer. Includes "
        "anomaly-score contour plots, a ROC curve, and a 3-way comparison."
    ),
    intro=(
        "`IsolationForest` has four hyperparameters that interact in non-obvious "
        "ways: `contamination` sets the decision threshold, `max_samples` controls "
        "how many points each tree sees, `max_features` determines which dimensions "
        "each tree uses, and `n_estimators` sets the ensemble size. The right values "
        "depend on the actual anomaly ratio and the structure of the data — a "
        "natural fit for evolutionary search.\n\n"
        "The key challenge for tuning any outlier detector is scoring: "
        "`IsolationForest` fits on `X` only (unsupervised), so the usual classifier "
        "scorers don't apply directly. Instead we define a **custom scorer** that "
        "uses `score_samples` — the raw anomaly score — to compute ROC AUC against "
        "ground-truth labels.\n\n"
        ":::info Relationship to the guide\n"
        "The [Outlier Detection guide](../guide/outliers) shows a minimal working "
        "example. This tutorial adds a realistic dataset, contour-plot "
        "visualizations, a ROC curve, and a full 3-way comparison against the "
        "default and random search.\n"
        ":::"
    ),
)
