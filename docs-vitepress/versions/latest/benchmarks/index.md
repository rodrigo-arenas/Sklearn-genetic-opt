...
The dataset has **real headroom** — a 3000×30 `make_classification` with a
non-linear, multi-cluster boundary that does *not* saturate near 1.0, so the
estimator family genuinely matters and there is room to separate.

<!-- CASH_RESULTS_START -->
*Results are generated by the command above and pasted here. Run it to reproduce on your hardware.*
<!-- CASH_RESULTS_END -->
...
- **Genetic search needs rugged spaces *and* budget *and* headroom — together.** Our own runs show that a mixed/categorical space alone is not enough to beat a strong Bayesian baseline when the data saturates or the budget is small. Evolutionary search and its optimizer controls (diversity, fitness sharing, local refinement) pull ahead when the objective is genuinely multimodal, there are enough generations to exploit it, and the dataset has real headroom — e.g. larger combined algorithm-selection-and-tuning problems. See [When to Use](../guide/when-to-use).
