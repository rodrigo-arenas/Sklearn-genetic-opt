...
def create_outlier_dataset():
    X_normal, _ = make_blobs(
        n_samples=200, centers=1, n_features=4, cluster_std=1.0, random_state=42
    )
    np.random.seed(42)
...
def test_isolation_forest_gasearch():
    ...
    param_grid = {"contamination": Continuous(0.05, 0.3), "n_estimators": Integer(50, 150)}
...
def test_one_class_svm_gasearch_default_scoring():
    ...
    param_grid = {"nu": Continuous(0.01, 0.5), "gamma": Categorical(["scale", "auto"])}
...
class MockDecisionFunctionOnlyEstimator(BaseEstimator):
...
def test_import_error_fallback():
    ...
    class MockOutlierEstimator(BaseEstimator):
        def __init__(self, param_value=0.5):
            self.param_value = param_value
            self.is_fitted_ = False
...
class MockFitPredictOnlyDetector(BaseEstimator):
...
def test_one_class_svm_gasearch_custom_scoring():
    ...
    param_grid = {"nu": Continuous(0.01, 0.5), "gamma": Categorical(["scale"])}
...
def test_local_outlier_factor_gasearch():
    ...
    param_grid = {"n_neighbors": Integer(5, 30), "contamination": Continuous(0.05, 0.3)}
...
def test_outlier_detection_with_y_none():
    ...
    param_grid = {"contamination": Continuous(0.05, 0.3), "n_estimators": Integer(50, 100)}
...
def test_outlier_detection_feature_selection_with_y_none():
    ...
    ga_feature_selection = GAFeatureSelectionCV(
        estimator=estimator, cv=3, population_size=4, generations=3, verbose=False, n_jobs=1
    )
...
def test_error_handling_invalid_estimator():
    ...
    param_grid = {"n_clusters": Integer(2, 5)}

    with pytest.raises(ValueError) as excinfo:
        ga_search = GASearchCV(estimator=estimator, param_grid=param_grid, cv=3)
...
def test_cv_results_structure():
    ...
    param_grid = {"contamination": Continuous(0.05, 0.2), "n_estimators": Integer(50, 100)}
...
def test_kfold_cv_for_outlier_detectors():
    ...
    param_grid = {"contamination": Continuous(0.05, 0.2)}
